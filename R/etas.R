                 ...
                 verbose = TRUE, plot.it = TRUE, no.itr = 11, covr)
{
  ...
  for (itr in 1:no.itr)
  {
    bkg <- decluster(param1, rbwd, revents, rpoly, tperiod, covr)
    revents <- bkg$revents
    integ0 <- bkg$integ0
    bk <- revents[,6]
    pb <- revents[,7]
    ...
    if (plot.it)
    {
      ...
      rates0 <- rates(param1, revents, win, tperiod, rbwd, plot.it=plot.it, covr = covr)
    }
    opt <- etasfit(param1, revents, rpoly, tperiod, integ0, verbose, covr)
