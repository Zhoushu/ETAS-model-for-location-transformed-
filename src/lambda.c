  ...
  for (register int i = 0; i < j; i++)
    {
      ...
      sig = D * exp(gamma * m[i]);
      r2 = dist2(x[j], y[j], x[i], y[i], sdX, sdY, corrXY);
              ...
              double corrXY)
{
  ...
  si = polyinteg(fr, w, np, px, py, x[j], y[j], sdX, sdY, corrXY);
  sk = A * exp(alpha * m[j]);
  return sk * gi * si;        
}
          ...
          double corrXY)
{
  ...
  si      = polyinteg(fr, w, np, px, py, x[j], y[j], sdX, sdY, corrXY);
  sid     = polyinteg(dD_fr, w, np, px, py, x[j], y[j], sdX, sdY, corrXY);
  siq     = polyinteg(dq_fr, w, np, px, py, x[j], y[j], sdX, sdY, corrXY);
  sigamma = polyinteg(dgamma_fr, w, np, px, py, x[j], y[j], sdX, sdY, corrXY);
             ...
             SEXP covr)
{
  ...
  double tt = *REAL(rt), xx = *REAL(rx), yy = *REAL(ry), sdX1 = cov[0], sdY1 = cov[1], corrXY1 = cov[2];
  ...
  while (t[i] < tt && i < N)
    {
      ...
      sig = D * exp(gamma * m[i]);
      r2 = dist2(xx, yy, x[i], y[i], sdX1, sdY1, corrXY1);
      part3 = (q - 1) / (sig * PI) * pow(1 + r2 / sig, - q);
